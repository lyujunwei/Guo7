...
@synthesize like_id,like_dataSorce;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    userimg = [[self.like_dataSorce objectAtIndex:indexPath.row]valueForKey:@"img_path"];
    ...
    userString = [[self.like_dataSorce objectAtIndex:indexPath.row]objectForKey:@"user"];
    ...
    contentString = [[self.like_dataSorce objectAtIndex:indexPath.row]objectForKey:@"content"];
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    Likeheader.backgroundColor=[UIColor clearColor];
    return Likeheader;
}

-(CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section{
    Likeheader =[[UIView alloc]initWithFrame:CGRectMake(0, 0, 320,10)];
    [self.view addSubview:Likeheader];
    return Likeheader.frame.size.height;
}
